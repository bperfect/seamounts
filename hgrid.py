...
class CGrid(object):
    ...
    def _calculate_subgrids(self):
        if isinstance(self.x_vert, np.ma.MaskedArray):
            self.x_rho = 0.25*(self.x_vert.data[1:,1:]+self.x_vert.data[1:,:-1]+ \
                           self.x_vert.data[:-1,1:]+self.x_vert.data[:-1,:-1])
            self.y_rho = 0.25*(self.y_vert.data[1:,1:]+self.y_vert.data[1:,:-1]+ \
                           self.y_vert.data[:-1,1:]+self.y_vert.data[:-1,:-1])
#        self.mask_rho = 0.25*(self.x_vert.mask[1:,1:]+self.x_vert.mask[1:,:-1]+ \
#                           self.x_vert.mask[:-1,1:]+self.x_vert.mask[:-1,:-1])
        
            self.x_u = 0.5*(self.x_vert.data[:-1,1:-1] + self.x_vert.data[1:,1:-1])
            self.y_u = 0.5*(self.y_vert.data[:-1,1:-1] + self.y_vert.data[1:,1:-1])
 #       self.mask_u = 0.5*(self.y_vert.mask[:-1,1:-1] + self.y_vert.mask[1:,1:-1])
        
            self.x_v = 0.5*(self.x_vert.data[1:-1,:-1] + self.x_vert.data[1:-1,1:])
            self.y_v = 0.5*(self.y_vert.data[1:-1,:-1] + self.y_vert.data[1:-1,1:])
  #      self.mask_v = 0.5*(self.y_vert.mask[1:-1,:-1] + self.y_vert.mask[1:-1,1:])
        
            self.x_psi = self.x_vert.data[1:-1,1:-1]
            self.y_psi = self.y_vert.data[1:-1,1:-1]
   #     self.mask_psi = self.y_vert.mask[1:-1,1:-1]
